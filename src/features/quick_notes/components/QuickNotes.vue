        ...
        <div class="space-y-3 w-full">
          <div class="flex items-center gap-3">
            ...
            <span class="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70">
              {{ formatDateHeader(String(dateKey)) }}
            </span>
          ...
          <div class="grid grid-cols-1 gap-2 w-full">
            <QuickNoteItem
              ...
              :key="note.id"
              :note="note"
              @toggle="todoStore.toggleQuickNote"
              @delete="handleDeleteQuickNote"
            />
          </div>
        </div>
      </template>
...
import QuickNoteItem from './QuickNoteItem.vue'
...
const addQuickNote = () => {
  if (quickNote.value.trim()) {
    ...
    quickNote.value = ''
    // DÜZELTME: Toast mesajı artık dil dosyasından (i18n) çekiliyor
    uiStore.addToast(t('quickNotes.noteAdded') || 'Note added successfully', 'success') 
  }
}

const handleDeleteQuickNote = async (noteId: string) => {
  // DÜZELTME: Confirm modal metinleri artık dil dosyasından (i18n) çekiliyor
  const isConfirmed = await uiStore.showConfirm(
    t('quickNotes.deleteConfirmTitle') || 'Delete Note', 
    t('quickNotes.deleteConfirmDesc') || 'Are you sure you want to delete this quick note?', 
  ...
  if (isConfirmed) {
    ...
    // DÜZELTME: Toast mesajı artık dil dosyasından (i18n) çekiliyor
    uiStore.addToast(t('quickNotes.noteDeleted') || 'Note deleted', 'info')
  }
}
